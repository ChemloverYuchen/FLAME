...
class AseEnvironmentProvider(BaseEnvironmentProvider):
    ...
    def get_environment(self, atoms, grid=None):
        ...
        if idx_i.shape[0] > 0:
            ...
            nbh_range = np.tile(
                np.arange(n_max_nbh, dtype=np.dtype('int'))[np.newaxis], (n_nbh.shape[0], 1)
            )

            mask = np.zeros((n_atoms, np.max(n_max_nbh)), dtype=np.dtype('bool'))
            mask[uidx, :] = nbh_range < n_nbh
            neighborhood_idx = -np.ones((n_atoms, np.max(n_max_nbh)), dtype=np.dtype('float32'))
            neighborhood_idx[mask] = idx_j

            offset = np.zeros((n_atoms, np.max(n_max_nbh), 3), dtype=np.dtype('float32'))
            offset[mask] = idx_S
        else:
            neighborhood_idx = -np.ones((n_atoms, 1), dtype=np.dtype('float32'))
            offset = np.zeros((n_atoms, 1, 3), dtype=np.dtype('float32'))
