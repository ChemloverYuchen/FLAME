...
from FLAME.train.schnet import MaxEpochHook, MaxStepHook, ReduceLROnPlateauHook, CSVHook, TensorboardHook, Trainer
from FLAME.train.schnet.metrics import MeanAbsoluteError, RootMeanSquaredError
from .settings  import  get_derivative, get_contributions, get_stress
import torch
...
def get_trainer(args, model, train_loader, val_loader, metrics):
    ...
    hooks = [MaxEpochHook(args.max_epochs)]
    if args.max_steps:
        hooks.append(MaxStepHook(max_steps=args.max_steps))

    schedule = ReduceLROnPlateauHook(
        optimizer=optimizer,
...
def get_metrics(args):
    ...
    metrics = [
        MeanAbsoluteError(args.property, args.property),
        RootMeanSquaredError(args.property, args.property),
    ]
    ...
    if stress is not None:
        metrics += [
            MeanAbsoluteError(stress, stress, element_wise=True),
            RootMeanSquaredError(stress, stress, element_wise=True),
        ]
