...
from FLAME.chemprop.args import TrainArgs
from FLAME.chemprop.utils import create_logger, makedirs, timeit, multitask_mean
from FLAME.dataprocess.chemprop.data import get_data, get_task_names, MoleculeDataset, validate_dataset_type
from FLAME.dataprocess.chemprop.features import set_extra_atom_fdim, set_extra_bond_fdim, set_explicit_h, set_adding_hs, set_keeping_atom_map, set_reaction, reset_featurization_parameters
...
def uvvisml_train(model_path, train_data, valid_data, test_data, epoch, config=None) -> None:
    """Parses Chemprop training arguments and trains (cross-validates) a Chemprop model.

    This is the entry point for the command line command :code:`abtmpnn_train`.
    """
    if config == None:
        config = f"{os.path.dirname(os.path.normpath(__file__))}/default.json"
    ...
    args = ['--data_path', train_data, '--separate_val_path', valid_data,
                 '--dataset_type', 'regression', '--save_dir', model_path, '--metric', 'mae',
                 '--epochs', str(epoch), '--config_path', config,
