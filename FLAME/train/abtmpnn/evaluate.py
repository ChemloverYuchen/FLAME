...
from FLAME.dataprocess.abtmpnn.data import MoleculeDataLoader, StandardScaler
from FLAME.abtmpnn.models import MoleculeModel
from FLAME.abtmpnn.utils import get_metric_func
...
def evaluate(model: MoleculeModel,
    ...
    :param metrics: A list of names of metric functions.
    :param dataset_type: Dataset type.
    :param scaler: A :class:`~abtmpnn.features.scaler.StandardScaler` object fit on the training targets.
    :param logger: A logger to record output.
    :return: A dictionary mapping each metric in :code:`metrics` to a list of values for each task.
