...
from FLAME.abtmpnn.args import TrainArgs
from FLAME.dataprocess.abtmpnn.data import get_data, get_task_names, MoleculeDataset, validate_dataset_type
from FLAME.abtmpnn.utils import create_logger, makedirs, timeit
from FLAME.dataprocess.abtmpnn.features import set_extra_atom_fdim, set_extra_bond_fdim
...
def cross_validate(args: TrainArgs,
    ...
    :param args: A :class:`~abtmpnn.args.TrainArgs` object containing arguments for
...
def abtmpnn_train(model_path, train_data, valid_data, test_data, epoch) -> None:
    ...
    train_args = ['--data_path', train_data, '--dataset_type', 'regression',
            '--separate_val_path', valid_data, '--metric', 'mae',
            '--save_dir', model_path, '--bond_fast_attention', '--atom_attention',
            '--normalize_matrices', '--no_features_scaling', '--epochs', str(epoch), '--number_of_molecules', '2']
