...
from FLAME.dataprocess.flsf.data import get_data, get_task_names, MoleculeDataset, validate_dataset_type
from FLAME.flsf.utils import create_logger, makedirs, timeit
...
def flsf_train(model_path, train_data, valid_data, test_data, epoch, ensemble=1, load='') -> None:
    ...
    train_args = ['--data_path', train_data, '--dataset_type', 'regression',
            ...
            '--save_dir', model_path, '--bond_fast_attention', '--atom_attention', '--ensemble_size', str(ensemble),
            '--normalize_matrices', '--no_features_scaling', '--epochs', str(epoch), '--number_of_molecules', '2']
    if os.path.exists(model_path):
        print('load existing model')
        train_args += ['--checkpoint_dir', model_path]
    elif len(load) > 0:
        train_args += ['--checkpoint_dir', load]
    else:
        print('create new model')
    cross_validate(args=TrainArgs().parse_args(train_args), train_func=run_training)
