...
# patterns = [(k,scaff_dict[v]) for v,k in dt]
patterns = [k for v,k in dt]
patterns1 = [(k,scaff_dict[v]) for v,k in dt]

def is_match(mol, patt):
    return int(mol.HasSubstructMatch(patt))

def make_scf_tag(mol, multi=False):
    if type(mol) == str:
        mol = Chem.MolFromSmiles(mol)
#     scfp = AllChem.GetMorganFingerprintAsBitVect(mol, 2, 728) #morgan
#     scfp = MACCSkeys.GenMACCSKeys(mol).ToList() #maccs
#     m_func = partial(is_match, mol)
#     scfp = np.array(list(map(m_func, patterns)))
    scfp = np.zeros(len(scaffold), dtype=int)
    for patt,idx in patterns1:
        if scfp[idx]==1:
            continue
...
#             if scfp[idx]==0 and mol.HasSubstructMatch(patt):
#                 scfp[idx] = 1
    return scfp