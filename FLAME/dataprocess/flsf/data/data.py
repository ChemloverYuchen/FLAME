...
from FLAME.dataprocess.flsf.features import get_features_generator
from FLAME.dataprocess.flsf.features import BatchMolGraph, MolGraph
...
def cache_graph() -> bool:
    r"""Returns whether :class:`~flsf.features.MolGraph`\ s will be cached."""
    return CACHE_GRAPH


def set_cache_graph(cache_graph: bool) -> None:
    r"""Sets whether :class:`~flsf.features.MolGraph`\ s will be cached."""
    global CACHE_GRAPH
...
class MoleculeDatapoint:
    ...
    def __init__(self,
        ...
        if self.features_generator is not None:
            ...
            for fg in self.features_generator:
                # flsf/features/features_generators.py
    ...
    def mol(self) -> List[Chem.Mol]:
        ...
        mol = [SMILES_TO_MOL.get(s, self.Smiles2Mol(s))
...
class MoleculeDataset(Dataset):
    ...
    def batch_graph(self) -> List[BatchMolGraph]:
        r"""
        Constructs a :class:`~flsf.features.BatchMolGraph` with the graph featurization of all the molecules.

        .. note::
           The :class:`~flsf.features.BatchMolGraph` is cached in after the first time it is computed
           and is simply accessed upon subsequent calls to :meth:`batch_graph`. This means that if the underlying
           set of :class:`MoleculeDatapoint`\ s changes, then the returned :class:`~flsf.features.BatchMolGraph`
           will be incorrect for the underlying data.

        :return: A list of :class:`~flsf.features.BatchMolGraph` containing the graph featurization of all the
        ...
        if self._batch_graph is None:
            ...
            # class BatchMolGraph: flsf/features/featurization.py
    ...
    def normalize_features(self, scaler: StandardScaler = None, replace_nan_token: int = 0,
        ...
        """
        Normalizes the features of the dataset using a :class:`~flsf.data.StandardScaler`.

        The :class:`~flsf.data.StandardScaler` subtracts the mean and divides by the standard deviation
        for each feature independently.

        If a :class:`~flsf.data.StandardScaler` is provided, it is used to perform the normalization.
        Otherwise, a :class:`~flsf.data.StandardScaler` is first fit to the features in this dataset
        and is then used to perform the normalization.

        :param scaler: A fitted :class:`~flsf.data.StandardScaler`. If it is provided it is used,
                       otherwise a new :class:`~flsf.data.StandardScaler` is first fitted to this
        ...
        :return: A fitted :class:`~flsf.data.StandardScaler`. If a :class:`~flsf.data.StandardScaler`
                 is provided as a parameter, this is the same :class:`~flsf.data.StandardScaler`. Otherwise,
                 this is a new :class:`~flsf.data.StandardScaler` that has been fit on this dataset.
        """
    ...
    def normalize_targets(self) -> StandardScaler:  # for regression tasks
        """
        Normalizes the targets of the dataset using a :class:`~flsf.data.StandardScaler`.
        ...
        :return: A :class:`~flsf.data.StandardScaler` fitted to the targets.
        """
...
def construct_molecule_batch(data: List[MoleculeDatapoint]) -> MoleculeDataset:
    ...
    Additionally, precomputes the :class:`~flsf.features.BatchMolGraph` for the constructed
