...
import FLAME.schnetpack.nn as L
...
class Atomwise(nn.Module):
    ...
    def __init__(
        ...
        n_layers=2,
        n_neurons=None,
        activation=L.activations.shifted_softplus,
        property="y",
        contributions=None,
        derivative=None,
        negative_dr=False,
        ...
        if outnet is None:
            self.out_net = nn.Sequential(
                L.base.GetItem("representation"),
                L.blocks.MLP(n_in, n_out, n_neurons, n_layers, activation),
            )
        else:
            self.out_net = outnet

        # build standardization layer
        self.standardize = L.base.ScaleShift(mean, stddev)

        # build aggregation layer
        if aggregation_mode == "sum":
            self.atom_pool = L.base.Aggregate(axis=1, mean=False)
        elif aggregation_mode == "avg":
            self.atom_pool = L.base.Aggregate(axis=1, mean=True)
        elif aggregation_mode == "max":
            self.atom_pool = L.base.MaxAggregate(axis=1)
        elif aggregation_mode == "softmax":
            self.atom_pool = L.base.SoftmaxAggregate(axis=1)
        else:
            raise AtomwiseError(
