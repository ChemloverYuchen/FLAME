...
# from FLAME.schnetpack.utils import read_deprecated_database
from FLAME.schnetpack import Properties
from FLAME.schnetpack.environment import SimpleEnvironmentProvider, collect_atom_triples
...
class AtomsData(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        properties["_idx"] = np.array([idx], dtype=np.dtype('int'))
...
def _convert_atoms(
    ...
    inputs[Properties.Z] = atoms.numbers.astype(np.dtype('int'))
    positions = atoms.positions.astype(np.dtype('float32'))
    ...
    inputs[Properties.neighbors] = nbh_idx.astype(np.dtype('int'))

    # Get cells
    inputs[Properties.cell] = np.array(atoms.cell.array, dtype=np.dtype('float32'))
    inputs[Properties.cell_offset] = offsets.astype(np.dtype('float32'))

    # If requested get neighbor lists for triples
    if collect_triples:
        nbh_idx_j, nbh_idx_k, offset_idx_j, offset_idx_k = collect_atom_triples(nbh_idx)
        inputs[Properties.neighbor_pairs_j] = nbh_idx_j.astype(np.dtype('int'))
        inputs[Properties.neighbor_pairs_k] = nbh_idx_k.astype(np.dtype('int'))

        inputs[Properties.neighbor_offsets_j] = offset_idx_j.astype(np.dtype('int'))
        inputs[Properties.neighbor_offsets_k] = offset_idx_k.astype(np.dtype('int'))
...
def torchify_dict(data):
    ...
    for pname, prop in data.items():
        ...
        if prop.dtype in [np.dtype('int'), np.dtype('int32'), np.dtype('int64')]:
#             pass
            torch_properties[pname] = torch.LongTensor(prop)
        elif prop.dtype in [np.dtype('float'), np.dtype('float32'), np.dtype('float64')]:
